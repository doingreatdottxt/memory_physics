    ...
    *new { arg context, doneCallback;
    ...
    alloc {
        ...
        SynthDef(\EnvChirps, { arg out;
            ...
            grove_trig = Dust.kr(e_val.linlin(0.01, 1, 0.0, 0.75));
            ...
            grove_sig = SinOsc.ar(grove_freq) * grove_env * e_val.linlin(0.01, 1, 0.0, 0.12);
            ...
            cave_freq = TExpRand.kr(900, 2600, cave_trig);
            ...
            swamp_trig = Dust.kr(e_val.linlin(0.01, 1, 0.0, 0.65));
            ...
            swamp_sig = LPF.ar(Saw.ar(TRand.kr(70, 190, swamp_trig)), 750) * swamp_env * e_val.linlin(0.01, 1, 0.0, 0.22);
        ...
        SynthDef(\MasterFX, { arg in, out;
            var sig, w_val, env_idx;
            var localIn, wetSig, delayTime, feedback, mod;
            var d_lpf, d_hpf, drive, driveSig;
            ...
            drive = Select.kr(env_idx % 8, [3.5, 5.0, 1.2, 1.0, 1.1, 1.5, 1.0, 1.0]);
            driveSig = (sig * drive).tanh * (1.0 / (drive.sqrt));
            sig = XFade2.ar(sig, driveSig, -0.4);
        ...
        SynthDef(\StrataLayer, { arg buf, out, depth=0, duration=2.0, t_reset=0, shift_offset=0, fade_time=4.0;
            ...
            var w_val, w_gate, env_idx, p_override, base_pressure, current_p, pressure;
        ...
        this.addCommand(\clear_layers, "", {
            ...
            recBuffer.zero;
            synthDepths = Array.fill(maxLayers, {arg i; i});
            synths.do({ arg syn, i; syn.set(\depth, synthDepths[i], \duration, 2.0, \t_reset, 1); });
        });
