...
local master_duration = -1 -- The Bedrock loop length

-- Stratigraphic Volume Targets (Layers 1 to 6)
...
function advance_strata()
  ...
  for i = 6, 2, -1 do
    ...
    layers[i].target_vol = BASE_VOLS[i] -- Update target for crossfade
  end
  
  layers[1].voice = recycled_voice
  layers[1].gain_mem = 1.0
  layers[1].duration = -1 
  layers[1].current_vol = 0.0 -- Fade in from zero
...
function stop_recording()
  ...
  elseif sync == 3 then 
    if master_duration > 0 then
      ...
      duration = Phys.snap_to_interval(duration, Phys.get_beat_sec()) -- Fallback for first loop
    end
  end
...
function audio_update_loop()
  ...
  while true do
    ...
    -- EROSION: Only the Surface (Layer 1) erodes
    if layers[1] and layers[1].duration > 0 and not is_recording then
      ...
      layers[1].gain_mem = math.max(0, layers[1].gain_mem - loss)
    end

    -- Update and Fade all voices
    for i, l in ipairs(layers) do
      ...
      -- Final volume combines environmental gain, frozen erosion state, and stratigraphic depth
      p.gain = p.gain * l.gain_mem * l.current_vol
